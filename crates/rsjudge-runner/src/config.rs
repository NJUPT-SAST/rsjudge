...
///
/// This owns the BPF bytecode generated by libseccomp in the parent process
/// (see [`ScmpFilterContext::export_bpf_mem`]), where allocation is
/// permitted. As a plain `Vec<u8>`, it is naturally [`Send`] + [`Sync`] and
/// can be moved into a `pre_exec` closure (see
/// [`std::os::unix::process::CommandExt::pre_exec`]).
///
/// Loading the bytecode in the forked child only requires two `prctl` calls
/// (`PR_SET_NO_NEW_PRIVS` and `PR_SET_SECCOMP`), both of which are
/// async-signal-safe and perform no allocation.
#[derive(Debug, Clone)]
...
impl SeccompFilter {
    ...
    pub fn from_config(config: &SeccompConfig, effective_caps: &CapSet) -> Result<Self> {
        ...
        let bpf = ctx.export_bpf_mem().map_err(io::Error::other)?;
        Ok(Self { bpf })
    }
    ...
    /// program via `PR_SET_SECCOMP`. It is intended
    /// to be called from a `pre_exec` hook in the forked child process.
    ///
    /// # Errors
    ///
    ...
    pub fn load(&self) -> io::Result<()> {
        ...
        let len = self.bpf.len() / std::mem::size_of::<sock_filter>();
        let mut fprog = sock_fprog {
            len: len as c_ushort,
            filter: self.bpf.as_ptr() as *mut sock_filter,
        };
